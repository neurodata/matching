...
from ...graspologic.graspologic.match.types import (
    AdjacencyMatrix,
    Int,
    MultilayerAdjacency,
    PaddingType,
...
def _compute_coefficients(
    ...
    for i in range(n_layers):
        ...
        # a_intra += product_trace(AR, BtRt)
        # b_intra += product_trace(A[i] @ Q, BtRt) + product_trace(AR, B[i].T @ Q.T)
        # b_intra += product_trace(A_ns[i].T @ R, B_ns[i]) + product_trace(
        #     A_sn[i] @ R, B_sn[i].T
        # )

    a = a_cross + a_intra
    b = b_cross + b_intra
    b += np.sum(S * R)  # equivalent to trace(S.T @ R)

    return a, b


def product_trace(X: np.ndarray, Y: np.ndarray):
    # trace(X@Y) = (X.T*Y).sum()
    return (X.T * Y).sum()
...
def _split_multilayer_matrix(
    matrices: MultilayerAdjacency, n: int, numba=False
) -> Tuple[
